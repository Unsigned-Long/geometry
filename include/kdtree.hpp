...
enum class SplitFeature { X, Y, Z };

/**
 * @brief Orderly exchange and partition dimensions in the composition of kdtree
 * construction for kdtree2
 *
...
/**
 * @brief Orderly exchange and partition dimensions in the composition of kdtree
 * construction for kdtree3
 *
 * @param sf the split feature
 * @return SplitFeature
 */
SplitFeature exfDime3(SplitFeature sf) {
  return (sf == SplitFeature::X
              ? SplitFeature::Y
              : (sf == SplitFeature::Y ? SplitFeature::Z : SplitFeature::X));
}

std::ostream &operator<<(std::ostream &os, SplitFeature sf) {
  os << (sf == SplitFeature::X ? "[X]"
                               : (sf == SplitFeature::Y ? "[Y]" : "[Z]"));
  return os;
}
...
template <typename _Ty>
bool isIntersect(const Point2<_Ty> &searchPoint, const Point2<_Ty> &nodePoint,
                 float radius, SplitFeature sf) {
  bool bl;
  switch (sf) {
    case SplitFeature::X:
...
template <typename _Ty>
bool isIntersect(const Point3<_Ty> &searchPoint, const Point3<_Ty> &nodePoint,
                 float radius, SplitFeature sf) {
  bool bl;
  switch (sf) {
    case SplitFeature::X:
...
struct Node {
 ...
 public:
  ...
  Node(const node_ptr &fn, const node_ptr &ln, const node_ptr &rn,
       const point_type &p, SplitFeature sf)
...
class KdTree {
 ...
 public:
  ...
  KdTree(exchange_sf ef, std::size_t size)
      : _root(nullptr), _ef(ef), _size(size) {}
   ...
   */
  void printKdTree(int mode = 1, std::ostream &os = std::cout,
                   bool el = true) const {
   ...
   */
  void nearestKSearch(const point_type &searchPoint, std::size_t K,
                      std::vector<point_type> &ps,
                      std::vector<float> &dis) const {
    if (K == 0 || this->_size == 0)
      return;
    else if (K >= _size) {
      /**
       * @brief if the K is greater than the size of the kdtree, than add all
       * points to the 'ps'
       */
      K = _size;
      ...
      for (int i = 0; i != K; ++i)
        ps.at(i) = nodes.at(i)->_p,
        dis.at(i) = distance(nodes.at(i)->_p, searchPoint);
    ...
    while (!searchPath.empty()) {
      ...
      if (record.size() < K) {
        // if the size of the 'record' is less than 'K', than add the point to
        // the 'record'
        record.push_back(std::make_pair(curNode->_p, curDis));
        // update the max distance
        if (furthestDis < curDis) furthestDis = curDis;
      } else {
      ...
      // if current node is the main path node and it is intersect with the
      // [search point] judged by the max distance
      if (mainPath.find(curNode) != mainPath.cend() &&
        ...
        for (const auto &elem : nodes) searchPath.push(elem);
      }
    }
   ...
   * @param dis the distance
   */
  void radiusSearch(const point_type &searchPoint, float radius,
                    std::vector<point_type> &ps,
                    std::vector<float> &dis) const {
    if (radius <= 0.0 || this->_size == 0) return;
    // according binary search rule, get the initial seatch path
    std::stack<node_ptr> searchPath;
    std::unordered_set<node_ptr> mainPath;
    node_ptr tempNode = this->_root;
    ...
    while (!searchPath.empty()) {
      ...
      if (d <= radius) ps.push_back(curNode->_p), dis.push_back(d);
 ...
 protected:
   ...
   */
  virtual void sortPointSet(SplitFeature sf, pointset_type &lps,
                            pointset_type &rps) = 0;

  void buildKdTree(node_ptr &np, SplitFeature sf, pointset_type lps,
                   pointset_type rps) {
  ...
  void nodesOnRootNode(const node_ptr &node,
...
class KdTree2 : public KdTree<_PointType> {
 ...
 public:
  ...
  KdTree2(const pointset_type &ps) : partent_type(exfDime2, ps.size()) {
    this->buildKdTree(ps);
  }
 ...
 private:
  virtual void buildKdTree(pointset_type ps) override {
    ...
    for (const auto &elem : ps) mean_x += elem.x(), mean_y += elem.y();
    mean_x /= ps.size(), mean_y /= ps.size();
    float stdevp_x = 0.0f, stdevp_y = 0.0f;
    for (const auto &elem : ps)
      stdevp_x += std::pow(elem.x() - mean_x, 2),
          stdevp_y += std::pow(elem.y() - mean_y, 2);
    ...
    // fn, ln, rn, p, sp
    this->_root =
        std::make_shared<node_type>(nullptr, nullptr, nullptr, mp, sf);
...
class KdTree3 : public KdTree<_PointType> {
 ...
 public:
  ...
  KdTree3(const pointset_type &ps) : partent_type(exfDime3, ps.size()) {
    this->buildKdTree(ps);
  }
 ...
 private:
  virtual void buildKdTree(pointset_type ps) override {
    ...
    for (const auto &elem : ps)
      stdevp_x += std::pow(elem.x() - mean_x, 2),
          stdevp_y += std::pow(elem.y() - mean_y, 2),
          stdevp_z += std::pow(elem.z() - mean_z, 2);
    ...
    if (stdevp_x <= stdevp_y && stdevp_z <= stdevp_y) {
      ...
      // split feature : Z
      std::sort(ps.begin(), ps.end(),
                [](const auto &p1, const auto &p2) { return p1.z() < p2.z(); });
      sf = SplitFeature::Z;
    } else {
      // split feature : X
      std::sort(ps.begin(), ps.end(),
...
class RefKdTree2 : public KdTree2<_PointType> {
 ...
 public:
  RefKdTree2() = delete;
  RefKdTree2(const refpointset_type &ps) : partent_type(trans(ps)) {}
   ...
   */
  void nearestKSearch(const point_type &searchPoint, std::size_t K,
                      std::vector<refpoint_type> &ps,
                      std::vector<float> &dis) const {
    return partent_type::nearestKSearch({0, searchPoint.x(), searchPoint.y()},
                                        K, ps, dis);
  }
  ...
  void radiusSearch(const point_type &searchPoint, float radius,
                    std::vector<refpoint_type> &ps,
                    std::vector<float> &dis) const {
    return partent_type::radiusSearch({0, searchPoint.x(), searchPoint.y()},
                                      radius, ps, dis);
  }
 ...
 protected:
  static std::vector<refpoint_type> trans(const refpointset_type &ps) {
    ...
    int index = 0;
    for (const auto &elem : ps) rps[index++] = elem.second;
    return rps;
  }
};
...
class RefKdTree3 : public KdTree3<_PointType> {
 ...
 public:
  ...
  RefKdTree3(const refpointset_type &ps) : partent_type(trans(ps)) {}
  ...
  void nearestKSearch(const point_type &searchPoint, std::size_t K,
                      std::vector<refpoint_type> &ps,
                      std::vector<float> &dis) const {
    return partent_type::nearestKSearch(
        {0, searchPoint.x(), searchPoint.y(), searchPoint.z()}, K, ps, dis);
  }
  ...
  void radiusSearch(const point_type &searchPoint, float radius,
                    std::vector<refpoint_type> &ps,
                    std::vector<float> &dis) const {
    return partent_type::radiusSearch(
        {0, searchPoint.x(), searchPoint.y(), searchPoint.z()}, radius, ps,
        dis);
  }
