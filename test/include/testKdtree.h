...
class TestKdTree2f : public testing::Test {
 protected:
  ...
  void TearDown() override { delete _kdtree; }
...
TEST_F(TestKdTree2f, nearestKSearch) {
  ...
  EXPECT_TRUE(vec.cend() != std::find_if(vec.cbegin(), vec.cend(),
                                         [](const ns_geo::Point2f &p) {
                                           return p.x() == 1.0f &&
                                                  p.y() == 1.0f;
                                         }));
  EXPECT_TRUE(vec.cend() != std::find_if(vec.cbegin(), vec.cend(),
...
class TestKdTree3f : public testing::Test {
 protected:
  virtual void SetUp() override {
    ns_geo::PointSet3f ps{{0.0f, 2.0f, 0.0f},  {1.0f, 1.0f, 0.0f},
...
TEST_F(TestKdTree3f, nearestKSearch) {
  ...
  EXPECT_TRUE(
      vec.cend() !=
      std::find_if(vec.cbegin(), vec.cend(), [](const ns_geo::Point3f &p) {
...
TEST_F(TestRefKdTree2f, nearestKSearch) {
  ...
  EXPECT_TRUE(
      vec.cend() !=
      std::find_if(vec.cbegin(), vec.cend(), [](const ns_geo::RefPoint2f &p) {
...
TEST_F(TestRefKdTree3f, nearestKSearch) {
  ...
  EXPECT_TRUE(
      vec.cend() !=
      std::find_if(vec.cbegin(), vec.cend(), [](const ns_geo::RefPoint3f &p) {
